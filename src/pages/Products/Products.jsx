...
const Products = () => {
  ...
  useEffect(() => {
    ...
    const getProductsforthiscategory = async (category, fetchedCategory, displayName = fetchedCategory, cursor = null) => {
      try {
        setLoading(cursor ? false : true);
        setIsLoadingMore(cursor ? true : false);
        
        const response = await config.getProducts(category, fetchedCategory, 10, cursor);
        ...
        if (response) {
          ...
          if (cursor) {
            ...
            dispatch(productFetch([...productDatas, ...products]));
          } else {
            // Replace with new products on first load
            dispatch(productFetch(products));
          }
          
          setNextCursor(pagination.nextCursor);
          setHasMoreProducts(pagination.hasMore);
    ...
    const getAllProducts = async (cursor = null) => {
      try {
        setLoading(cursor ? false : true);
        setIsLoadingMore(cursor ? true : false);
        
        const response = await config.getallProducts(10, cursor);
    ...
    const handleLoadMore = () => {
      if (categoryName === "All Products") {
        getAllProducts(nextCursor);
      } else if (categoryName === "Sale") {
        getProductsforthiscategory("discountOption", "yes", "Sale", nextCursor);
      } else if (categoryName === "Our Best Products") {
        getProductsforthiscategory("isFeatured", "yes", "Our Best Products", nextCursor);
      } else {
        getProductsforthiscategory("category", categoryName, categoryName, nextCursor);
      }
    };
        ...
        <div className="pt-48 flex justify-start items-start">
          ...
          <div className="w-full sm:w-[75%]">
                  ...
                  <div className="flex justify-between mt-8">
                    <div onClick={handleMenuBar} className=" flex flex-col gap-6 mt-4 ">
                      <div className="text-dark">
                        ...
                        <ul className="mt-4 cursor-pointer" onClick={() => getProductsforthiscategory("discountOption", "yes", "Sale")}>Products on Sale</ul>
                        <ul className="mt-4 cursor-pointer" onClick={() => getProductsforthiscategory("isFeatured", "yes", "Our Best Products")}>Featured Products</ul>
                      </div>
                    </div>
                    <div className="cursor-pointer">
                      <RxCross1 size={"40px"} onClick={handleMenuBar} />
            ...
            <div className="flex flex-wrap gap-4 justify-center items-start">
              {productDatas.map(function (item) {
                return (
                  <div key={item.id} data-aos="fade-up" onClick={() => {gotosingleProduct(item.id)}}>
                    <ProductCard
                      image={item.images[0]}
                      title={item.title}
                      discountedPrice={item.discountedPrice}
